...
class LevelZeroSpec extends FlatSpec with OptionValues {
  ...
  "A simple level zero chess A.I" should "generate a random move" in {
    val defaultBoard = Board.defaultBoard()
    val player       = White
    val history      = Seq()
    val ai           = AI(0)
    println(ai.generateSmartMove(defaultBoard, White, history).value)
    new MoveGenerator(defaultBoard, player, history)
      .allMoves()
      .contains(ai.generateSmartMove(defaultBoard, White, history).value)
  }
